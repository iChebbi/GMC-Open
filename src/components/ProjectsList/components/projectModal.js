...
export default ({ currentProject, toggleModal }) => {
  const { product: { name, description, logo, image, keywords, url } = {}, owner: { email_Value: email } = {} } = currentProject
  return (
        ...
        <div className="project-header">
          ...
          <div className="text-content">
            ...
            <div className="keywords">
              {keywords && JSON.parse(keywords).map((keyword, key) => <div key={key} className="keyword">{keyword.text}</div>)}
            </div>
          </div>
          <div>
